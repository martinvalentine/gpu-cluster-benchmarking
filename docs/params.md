...
| ---------------------- | ------ | -------- | ------------------------------------------------------------ |
| `cluster.gpu_count`    | int    | `0`      | Number of GPUs. `0` = auto-detect from `nvidia-smi`         |
| `cluster.vllm.tp`      | int    | `6`      | Tensor parallel size — GPUs used for model sharding          |
| `cluster.vllm.gpu_mem_util` | float | `0.92` | Fraction of GPU VRAM to reserve (0.92 = ~44 GB on 48 GB A40) |
| `cluster.vllm.max_model_len` | int | `4096`  | Maximum context window per request (tokens)                  |
| `cluster.vllm.max_num_seqs` | int  | `512`   | Maximum concurrent sequences the server handles              |
| `cluster.vllm.max_batched_tokens` | int | `16384` | Max tokens per batch (chunked prefill)                |
...
| `cluster.vllm.swap_space` | int | `0`      | CPU swap space in GB (0 = disabled, use RAM as overflow)     |
...
| Parameter                    | Type   | Default  | Description                                                 |
| ---------------------------- | ------ | -------- | ----------------------------------------------------------- |
| `cluster.llamacpp.n_parallel` | int   | `16`     | Concurrent request slots (each gets its own KV cache)       |
| `cluster.llamacpp.ctx_size`  | int   | `8192`   | Context window per slot (total KV = n_parallel × ctx_size)  |
| `cluster.llamacpp.n_gpu_layers` | string | `all`  | GPU offload layers: `all` (=999), or exact number           |
| `cluster.llamacpp.tensor_split` | string | `""`   | GPU split ratios: `""` = single GPU, `"1,1,1,1,1,1"` = 6 GPU |
| `cluster.llamacpp.batch`    | int    | `4096`   | Prefill batch size (tokens per forward pass)                |
| `cluster.llamacpp.ubatch`   | int    | `1024`   | Micro-batch size (subdivides batch for GPU kernels)         |
| `cluster.llamacpp.threads`  | int    | `32`     | CPU threads for prefill/processing                          |
...
| `backend`    | string | `vllm`, `llamacpp`                 | Which serving framework to use                  |
| `proxy_name` | string | unique string                      | LiteLLM proxy route name                        |
| `endpoint`   | bool   | `true`, `false`                    | Whether to register in LiteLLM proxy            |
| `api_port`   | int    | port number                        | Override backend default port (e.g. 8003)       |
...
| `dataset.sharegpt` | string | `/workspace/datasets/sharegpt.json` | Benchmark dataset path                   |
...
## Python Parser — Exact YAML Keys Read

The `parse_models()` function in `bench-models.sh` reads these exact keys:

```
Per-model vLLM:   vllm_tp, vllm_gpu_mem, vllm_quant, vllm_max_seqs,
                  vllm_max_model_len, vllm_max_batched_tokens,
                  vllm_block_size, vllm_dtype, vllm_swap_space,
                  vllm_distributed_executor, vllm_enforce_eager

Per-model llama:  llamacpp_n_parallel, llamacpp_ctx_size,
                  llamacpp_n_gpu_layers, llamacpp_tensor_split,
                  llamacpp_batch, llamacpp_ubatch, llamacpp_threads,
                  llamacpp_cache_key, llamacpp_cache_val,
                  llamacpp_flash_attn, llamacpp_cache_prompt

Cluster vllm:     tp, gpu_mem_util, max_model_len, max_num_seqs,
                  max_batched_tokens, quant, block_size, dtype,
                  swap_space, enforce_eager, distributed_executor

Cluster llama:    n_parallel, ctx_size, n_gpu_layers, tensor_split,
                  batch, ubatch, threads, cache_key, cache_val,
                  flash_attn, cache_prompt
```
